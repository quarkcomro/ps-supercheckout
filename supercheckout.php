...
class Supercheckout extends Module
{
    ...
    /*
     * Main tab was giving error in the admin panel, so updated the parent tab class to avoid the error
     * @modifier Himanshu Vishwakarma
     * @date 04-07-2025
     */
    const PARENT_TAB_CLASS = 'AdminKBParentSupercheckout';
    const SELL_CLASS_NAME = 'SELL';
    private $supercheckout_controllers = array(
        'AdminSuperSetting' => array(
            'name' => 'supercheckout',
            'tab' => 'checkout',
            'version' => '9.0.8',
            'author' => 'Knowband',
            'need_instance' => 0,
            'module_key' => '68a34cdd0bc05f6305874ea844eefa05',
            'author_address' => '0x2C366b113bd378672D4Ee91B75dC727E857A54A6',
            'ps_versions_compliancy' => array('min' => '1.7.0.0', 'max' => _PS_VERSION_),
            'bootstrap' => true,
        ),
    );
    ...
    public function __construct()
    {
        ...
        if (!class_exists('MailChimpKbSC')) {
            include_once dirname(__FILE__) . '/vendor/mailchimp-api/src/MailChimpKbSC.php';
        }
    ...
    public function cleanUpAllTabs()
    {
        $classes_to_delete = array(
            self::PARENT_TAB_CLASS,
            'AdminkbsupercheckoutConfigure'
        );
        $admin_menus = $this->adminSubMenus();
        foreach ($admin_menus as $menu) {
            $classes_to_delete[] = $menu['class_name'];
        }

        foreach (array_unique($classes_to_delete) as $class_name) {
            $sql = 'SELECT id_tab FROM `' . _DB_PREFIX_ . 'tab` WHERE class_name = "' . pSQL($class_name) . '"';
            $tab_ids = Db::getInstance()->executeS($sql);
            if ($tab_ids) {
                foreach ($tab_ids as $id) {
                    if ($id['id_tab'] > 0) {
                        $tab = new Tab((int) $id['id_tab']);
                        $tab->delete();
                    }
                }
            }
    ...
    public function install()
    {
        ...
        if (!parent::install()
             ...
             * Registering Hook to show the fee on the Admin Order Page
             * @date 15-10-2024
            ...
            /**
             * Changes added to register the actionDispatcher hook to our module.
        ...
        /** Start Changes to add the Payment Fee on Checkout
         * Creating table kb_supercheckout_payment_fee to store the fee of the order
         * @date 15-10-2024
         * NAOct2024 payment_fee
         * @modifier Nikhil Aggarwal 
         */
        $payment_fee_sql = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'kb_supercheckout_payment_fee` (
            `id_payment_fee` INT(11) NOT NULL AUTO_INCREMENT,
            `id_order` INT(11) NOT NULL,
            `id_cart` INT(11) NOT NULL,
            `id_module` text NOT NULL,
            `paymentfee_applicable` VARCHAR(255) NOT NULL,
            PRIMARY KEY (`id_payment_fee`)
        ) ENGINE=' . _MYSQL_ENGINE_ . ' DEFAULT CHARSET=utf8;';
        // Changes end by Nikhil
        Db::getInstance()->execute($payment_fee_sql);
        // </editor-fold>
    ...
    public function uninstall()
    {
        ...
        if (!parent::uninstall()
            ...
            /**
             * Changes added to unregister the actionDispatcher hook to our module on uninstallation
             * @modifier Ashish Kumar
             * @date 06-05-2024
             * AKMay2024 stripe-checkout-compatible
             */
            ...
            /** Start Changes to add the Payment Fee on Checkout
             * Unregistering Hook used to show the fee on the Admin Order Page
             * @date 15-10-2024
             * NAOct2024 payment_fee
             * @modifier Nikhil Aggarwal 
             */
    ...
    public function getModuleTranslationByLanguage($module, $string, $source, $language, $sprintf = null, $js = false)
    {
        ...
        if (isset($current_key_file) && !empty($modules[$current_key_file])) {
            $ret = stripslashes($modules[$current_key_file]); // Updated for PS9: Tools::stripslashes() removed, using native PHP function
        } elseif (isset($default_key_file) && !empty($modules[$default_key_file])) {
            $ret = stripslashes($modules[$default_key_file]); // Updated for PS9: Tools::stripslashes() removed, using native PHP function
        } elseif (!empty($modules[$current_key])) {
            $ret = stripslashes($modules[$current_key]); // Updated for PS9: Tools::stripslashes() removed, using native PHP function
        } elseif (!empty($modules[$default_key])) {
            $ret = stripslashes($modules[$default_key]); // Updated for PS9: Tools::stripslashes() removed, using native PHP function
            // if translation was not found in module, look for it in AdminController or Helpers
        } elseif (!empty($langadm)) {
            $ret = stripslashes(Translate::getGenericAdminTranslation($string, $key, $langadm)); // Updated for PS9: Tools::stripslashes() removed, using native PHP function
        } else {
            $ret = stripslashes($string); // Updated for PS9: Tools::stripslashes() removed, using native PHP function
        }
    ...
    protected function unInstallKbTabs()
    {
        if (version_compare(_PS_VERSION_, '1.7', '<')) {
             ...
             */
            // Delete parent tab(s)
            $parent_tab_ids_query = 'SELECT id_tab FROM `' . _DB_PREFIX_ . 'tab` WHERE class_name = "' . pSQL(self::PARENT_TAB_CLASS) . '"';
            $parent_tab_ids = Db::getInstance()->executeS($parent_tab_ids_query);
            if ($parent_tab_ids) {
                foreach ($parent_tab_ids as $id) {
                    $tab = new Tab($id['id_tab']);
                    $tab->delete();
                }
            }

            // Also try to delete the *old* parent tab if it exists
            $old_parent_tab_class = 'AdminkbsupercheckoutConfigure';
            $old_parent_tab_ids_query = 'SELECT id_tab FROM `' . _DB_PREFIX_ . 'tab` WHERE class_name = "' . pSQL($old_parent_tab_class) . '"';
            $old_parent_tab_ids = Db::getInstance()->executeS($old_parent_tab_ids_query);
            if ($old_parent_tab_ids) {
                foreach ($old_parent_tab_ids as $id) {
                    $tab = new Tab($id['id_tab']);
                    $tab->delete();
                }
            }

            // Delete the problematic duplicate tab
            $old_duplicate_tab_class = 'AdminSuperSetting';
            $old_duplicate_tab_ids_query = 'SELECT id_tab FROM `' . _DB_PREFIX_ . 'tab` WHERE class_name = "' . pSQL($old_duplicate_tab_class) . '"';
            $old_duplicate_tab_ids = Db::getInstance()->executeS($old_duplicate_tab_ids_query);
            if ($old_duplicate_tab_ids) {
                foreach ($old_duplicate_tab_ids as $id) {
                    // Check if tab is a parent tab without a controller action
                    $tab = new Tab($id['id_tab']);
                    $parent_id = Tab::getIdFromClassName(self::SELL_CLASS_NAME);
                    if ($tab->id_parent == $parent_id) {
                        $tab->delete();
                    }
                }
            }

            // Delete sub-menu tabs
            $admin_menus = $this->adminSubMenus();
            foreach ($admin_menus as $menu) {
                $menu_tab_ids_query = 'SELECT id_tab FROM `' . _DB_PREFIX_ . 'tab` WHERE class_name = "' . pSQL($menu['class_name']) . '" AND module = "' . pSQL($this->name) . '"';
                $menu_tab_ids = Db::getInstance()->executeS($menu_tab_ids_query);
                if ($menu_tab_ids) {
                    foreach ($menu_tab_ids as $id) {
                        $tab = new Tab($id['id_tab']);
                        $tab->delete();
                    }
                }
    ...
    public function getContent()
    {
            ...
            if (Tools::isSubmit('method')) {
                ...
                switch (Tools::getValue('customer_profile_action')) {
                    ...
                    case 'displayEditCustomerProfileForm':
                        ...
                        /*
                         * Ensure custom_fields index exists for older profiles to avoid undefined array key notice on PHP 8+
                         * 26-02-2026
                         */
                        if (!isset($edit_profile_data['custom_fields']) || !is_array($edit_profile_data['custom_fields'])) {
                            $edit_profile_data['custom_fields'] = array();
                        }
                        foreach ($result_custom_fields_details as $custom_field_row) {
                            $custom_field_id = (int) $custom_field_row['id_velsof_supercheckout_custom_fields'];
                            if (!array_key_exists($custom_field_id, $edit_profile_data['custom_fields'])) {
                                $edit_profile_data['custom_fields'][$custom_field_id] = 0;
                            }
                        }
        ...
        if (Tools::isSubmit($this->submit_action . $this->name)) {
            ...
            /**
             * Start Changes to remove the use of HTML in PHP files
             * Removing implode function and $this->custom_errors code as it is not used
             * NANov2023 html_php
             * @date 30-11-2023
             * @modifier Nikhil Aggarwal
             */
            // if (count($this->custom_errors) > 0) {
            // $output .= $this->displayError($this->custom_errors);
            // CHanges end by Nikhil
            // } else {
            ...
            if (!is_array($payment_post_data)) {
                $payment_post_data = array();
            }
            if (!isset($payment_post_data['payment_method']) || !is_array($payment_post_data['payment_method'])) {
                $payment_post_data['payment_method'] = array();
            }
            $previous_payment_data = json_decode(Configuration::get('VELOCITY_SUPERCHECKOUT_DATA'), true);
            if (!is_array($previous_payment_data)) {
                $previous_payment_data = array();
            }
            foreach (PaymentModule::getInstalledPaymentModules() as $paymethod) {
                $id = (int) $paymethod['id_module'];
                if (!isset($payment_post_data['payment_method'][$id])) {
                    $payment_post_data['payment_method'][$id] = array();
                }
                if (!array_key_exists('sort_order', $payment_post_data['payment_method'][$id])) {
                    if (isset($previous_payment_data['payment_method'][$id]['sort_order'])) {
                        $payment_post_data['payment_method'][$id]['sort_order'] = $previous_payment_data['payment_method'][$id]['sort_order'];
                    } else {
                        $payment_post_data['payment_method'][$id]['sort_order'] = '';
                    }
                }
            }
            /**
             * Start Changes to remove the use of HTML in PHP files
             * Changing the type of $payment_error from string to array
             * NANov2023 html_php
             * @date 30-11-2023
             * @modifier Nikhil Aggarwal
             */
            ...
            foreach (PaymentModule::getInstalledPaymentModules() as $paymethod) {
                ...
                if (isset($_FILES['velocity_supercheckout_payment']['size']['payment_method'][$id]['logo']['name']) && $_FILES['velocity_supercheckout_payment']['size']['payment_method'][$id]['logo']['name'] == 0) {
                    ...
                    if (isset($_FILES['velocity_supercheckout_payment']['size']['payment_method'][$id]['logo']['name'])) {
                        ...
                        /**
                         * Start Changes to fix incorrect error messages for file validation
                         * Split the combined size+extension check into separate conditions
                         * so each failure shows the correct specific error message.
                         * @date 19-04-2026
                         * @modifier Manish
                         * MPAPR2026 WrongErrorMessage
                         */
                        if (!in_array($extension, $allowed_exts)) {
                            $image_error = $this->l('Error Uploaded file is not an image');
                            $payment_error[] = $image_error . " " . $paymethod['name'];
                        } elseif ($img_size >= 300000) {
                            $image_error = $this->l('Error Uploaded file exceeds the maximum allowed size of 300KB');
                            $payment_error[] = $image_error . " " . $paymethod['name'];
                        } else {
                            ...
                            if ($error > 0) {
                                ...
                                /**
                                 * Start Changes to remove the use of HTML in PHP files
                                 * Adding the error at an index of $payment_error
                                 * NANov2023 html_php
                                 * @date 30-11-2023
                                 * @modifier Nikhil Aggarwal
                                 */
            ...
            foreach ($carriers as $deliverymethod) {
                ...
                if ($method_file['size']['delivery_method'][$id]['logo']['name'] == 0) {
                    ...
                    if (!in_array($extension, $allowed_exts)) {
                    ...
                    } elseif ($method_file['size']['delivery_method'][$id]['logo']['name'] >= 300000) {
                        $file_error_msg = $this->l('Error Uploaded file exceeds the maximum allowed size of 300KB');
                        $payment_error[] = $file_error_msg . " " . $deliverymethod['name'];
                    } else {
                        if ($method_file['error']['delivery_method'][$id]['logo']['name'] > 0) {
                            ...
                            $payment_error[] = 'Error in image of ' . $deliverymethod['name'];
                            // CHanges end by Nikhil
                        } else {
                            ...
                            if (copy(
                                ...
                                $payment_error[] = 'Error in uploading the image of ' . $deliverymethod['name'];
                                // CHanges end by Nikhil
                            }
                            if (!version_compare(_PS_VERSION_, '1.6.0.1', '<')) {
                                Tools::chmodr(_PS_MODULE_DIR_ . 'supercheckout/views/img/uploads', 0755);
            ...
            /**
             * Start Changes to fix the issue of error block being shown even when no error is there.
             * This is occurring because we were checking that $payment_error is blank string or not. However, it is now being used as an array
             * Using !empty instead of != ''
             * NANov2023 $payment_error_array
             * @date 01-12-2023
             * @modifier Nikhil Aggarwal
             */
        ...
        if (!isset($paymentdata['payment_method']) || !is_array($paymentdata['payment_method'])) {
            $paymentdata['payment_method'] = array();
        }
        foreach (PaymentModule::getInstalledPaymentModules() as $pm) {
            $mid = (int) $pm['id_module'];
            if (!isset($paymentdata['payment_method'][$mid])) {
                $paymentdata['payment_method'][$mid] = array();
            }
            if (!array_key_exists('sort_order', $paymentdata['payment_method'][$mid])) {
                $paymentdata['payment_method'][$mid]['sort_order'] = '';
            }
        }
        $custombutton = array(
            'button_color' => '5CB85C',
        ...
        $total_price_method_arr = array(
            array(
                'id_method' => 0,
                'display_name' => $this->l('Default Prestshop Price Setting')),
            array(
                'id_method' => 1,
                'display_name' => $this->l('Tax Inclusive Price')),
            array(
                'id_method' => 2,
                'display_name' => $this->l('Tax Exclusive Price')),
            array(
                'id_method' => 3,
                'display_name' => $this->l('Both Inclusive and Exclusive Price'))
        );
        ...
        /* Start Changes to add the Payment Fee on Checkout
         * Passing the customer groups present on the store to the form
         * @date 08-10-2024
         * NAOct2024 payment_fee
         * @modifier Nikhil Aggarwal */
        $customer_groups = array();
        $sql = "SELECT id_group, name FROM " . _DB_PREFIX_ . "group_lang WHERE id_lang = '" . (int) $this->context->language->id . "'";
        $customer_group_kbcf = Db::getInstance()->executeS($sql);
        foreach ($customer_group_kbcf as $group_kbcf) {
            $customer_groups[] = array(
                'id_group' => $group_kbcf['id_group'],
                'name' => $group_kbcf['name']
            );
        }
    ...
    public function hookDisplayHeader()
    {
        ...
        if ($show) {
            if (!Tools::getValue('klarna_supercheckout')) {
                if (isset($settings['super_test_mode']) && $settings['super_test_mode'] != 1) {
                    ...
                    /**
                     * Made the changes for the Test Mode. If the test mode is disabled, then multiple redirect issue occurs.
                     * @modifier Ashish Kumar
                     * @date 09-05-2024
                     * AKMay2024 stripe-checkout-compatible
                     */
                    if (Tools::getValue('module') != 'supercheckout' && ($page_name == 'order-opc' || $page_name == 'order' || $page_name == 'checkout') || (isset($settings['disable_cart_page']) && $settings['disable_cart_page'] == 1 && $page_name == 'cart' && !isset($_SERVER['HTTP_X_REQUESTED_WITH']))) {
        ...
        /**
         * Callinh includeStripeOfficialData function in display header to include stripe official data on supercheckout page
         * @modifier Ashish Kumar
         * @date 06-05-2024
         * AKMay2024 stripe-checkout-compatible
         */
        $this->includeStripeOfficialData();

    }

    /**
     * Function responsible to include the stripe official If the Stripe Official class is not already loaded,
     * Responsible to includes the file after checking if the Stripe Official module is installed and configured correctly
     * @author Ashish Kumar
     * @date 06-05-2024
     * AKMay2024 stripe-checkout-compatible
     */
    private function includeStripeOfficialData() {

        $orderPageNames = ['supercheckout'];
        ...
        if (Module::isInstalled('stripe_official') && Module::isEnabled('stripe_official')) {
            //Check wellConfigured Function.
            if (!class_exists("Stripe_official")) {
                include(_PS_MODULE_DIR_ . "stripe_official/stripe_official.php");
            }
            $stripeOfficial = new Stripe_official();
            if (!$stripeOfficial::isWellConfigured()) {
                return;
            }
        ...
        $amount = $stripeOfficial::isZeroDecimalCurrency($currency->iso_code) ? $amount : $amount * 100;
        ...
        if (version_compare(_PS_VERSION_, '1.7', '>=')) {
            ...
            if (_PS_USE_MINIFY_JS_) {
                $this->context->controller->registerJavascript(
                    'stripe_official-payments',
                    'modules/stripe_official/views/js/checkout.js'
                );
            } else {
                $this->context->controller->registerJavascript(
                    'stripe_official-payments',
                    'modules/stripe_official/views/js/extensions/checkout.js'
                );
            }
        ...
        $stripe_position = '';
        $stripe_layout = '';
        $use_new_ps_translation = '';
        if ($stripeOfficial->version >= 3.2) {
            $stripe_position = Configuration::get($stripeOfficial::POSITION);
            $stripe_layout = Configuration::get($stripeOfficial::LAYOUT);
            $use_new_ps_translation = 1;
        }

        // Javacript variables needed by Elements
        Media::addJsDef([
            'stripe_pk' => $stripeOfficial->getPublishableKey(),
            'stripe_merchant_country_code' => $merchantCountry->iso_code,
            'save_payment_method' => "off",
            ...
            'stripe_postcode_disabled' => Configuration::get($stripeOfficial::POSTCODE),
            'stripe_module_dir' => Media::getMediaPath(_PS_MODULE_DIR_ . "stripe_official"),
            ...
            'stripe_payment_elements_enabled' => Configuration::get($stripeOfficial::ENABLE_PAYMENT_ELEMENTS),
            'capture_method' => Configuration::get(Stripe_official::CATCHANDAUTHORIZE) ? 'manual' : 'automatic',
            'postcode' => $address->postcode,
            'stripe_theme' => Configuration::get($stripeOfficial::THEME),

            'stripe_position' => $stripe_position,
            'stripe_layout' => $stripe_layout,
            'use_new_ps_translation' => $use_new_ps_translation,
    ...
    public function hookDisplayOrderConfirmation($params = null)
    {
        ...
        /** Start Changes to add the Payment Fee on the Order confirmation page
         * Fetching the fee of the order and displaying it on the order confirmation page
         * @date 15-10-2024
         * NAOct2024 payment_fee
         * @modifier Nikhil Aggarwal 
         */
        ...
        if ($module_settings['enable'] == 1) {
            if (isset($params['order']->id)) {
                ...
                $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'kb_supercheckout_payment_fee WHERE id_order = ' . (int) $order_id;
                $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($sql);
                if (isset($result) && !empty($result) && isset($result['paymentfee_applicable']) && $result['paymentfee_applicable'] != 0) {
                    $fee = $result['paymentfee_applicable'];
                    $is_fee_applicable = 1;
                    $priceFormatter = new PriceFormatter();
                    $this->context->smarty->assign('fee', $priceFormatter->format($fee, (int) $params['order']->id_currency));
                }
            }
        ...
        /** Start Changes to add the Payment Fee on the Order confirmation page
         * Displaying the fee message on the order confirmation page
         * @date 15-10-2024
         * NAOct2024 payment_fee
         * @modifier Nikhil Aggarwal 
         */
        if ($is_fee_applicable == 1) {
            return $this->display(__FILE__, 'kbsupercheckout_fee_message.tpl');
        }
        // Changes end by Nikhil
    }
    ...
    protected function getMailchimpLists($mailchimp_api)
    {
        try {
             ...
             * @modifier Nikhil Aggarwal
             */
            $mchimp = new MailChimpKbSC($id);
            $arrchimp = ($mchimp->get('lists'));
            if ($arrchimp !== false) {
                foreach ($arrchimp["lists"] as $list) {
                    $listchimp[] = array(
                        'id' => $list["id"],
                        'name' => $list["name"]
                    );
    ...
    {
        /*
         * Fixed SendinBlue API integration issues for PS9 compatibility
         * @modifier Himanshu Vishwakarma
         * @date 04-07-2025
         */
        try {
            ...
            if (trim($apikey) != '' && $apikey !== null) {
                // First try the v3 API directly as it's more reliable
                $curl = curl_init();
                ...
                if ($http_code === 200 && $result_data) {
                    $result_data = json_decode($result_data, true);
                    if (isset($result_data['lists']) && is_array($result_data['lists'])) {
                        foreach ($result_data['lists'] as $list) {
                    ...
                    try {
                        ...
                        // Try to get all folders first
                        $folders = $mailin->get_folders(array('page' => 1, 'page_limit' => 50));

                        if (isset($folders['data']) && is_array($folders['data'])) {
                        ...
                        // If no lists found in folders, try direct lists API
                        if (empty($response)) {
                            $lists_data = $mailin->get_lists(array('page' => 1, 'page_limit' => 50));
                            if (isset($lists_data['data']) && is_array($lists_data['data'])) {
                                foreach ($lists_data['data'] as $list) {
                                    if (isset($list['id']) && isset($list['name'])) {
                                        $response[] = array(
                                            'id' => $list['id'],
                                            'name' => $list['name']
                        ...
                        error_log('SendinBlue v2 API error: ' . $e->getMessage());
                    }
                }
            }
            ...
            error_log('SendinBlue API error: ' . $e->getMessage());
            echo json_encode(array('false'));
        }
        die;
    }
     ...
     */
    /*
     * Updated the function since the API request URL has been updated by klaviyo
     * @modifier Himanshu Vishwakarma
     * @date 04-07-2025
     */
    protected function getKlaviyoList($apiKey)
    {
        try {
            ...
            if ($httpCode !== 200) {
                $reason = isset($output->detail) ? $output->detail : $this->l('Unable to fetch Klaviyo lists.');
                echo json_encode([
                    'error' => [[
                        'value' => "0",
                        'label' => $reason
            ...
            $lists = array_filter($output->data ?? [], function ($list) {
                return $list->type === 'list';
            });

            usort($lists, function ($a, $b) {
                return Tools::strtolower($a->attributes->name) > Tools::strtolower($b->attributes->name) ? 1 : -1;
            });
            ...
            echo json_encode(['error' => [[
                'value' => "0",
                'label' => $this->l('Unexpected error occurred.')
            ]]]);
        }
        die;
    }
    ...
    public function addNewCustomField($custom_field_form_values)
    {
        ...
        $validation_type = isset($custom_field_form_values['validation_type']) ? $custom_field_form_values['validation_type'] : '';
    ...
    public function processMultilangFieldValues($arary_filed_values)
    {
        ...
        $flag_first = 0;
        /*
         * Fix undefined variable $default_label_value in processMultilangFieldValues
         * @modifier Himanshu Vishwakarma
         * @date 04-07-2025
         */
        $default_label_value = '';
    ...
    public function getFieldOptions($id_velsof_supercheckout_custom_fields)
    {
        ...
        foreach ($result_custom_fields_details_field_options as $lang_data) {
            ...
            /*
             * Initialize option string per language to avoid undefined array key notice on PHP 8+
             * 26-02-2026
             */
            if (!isset($array_fields_options[$lang_data['id_lang']])) {
                $array_fields_options[$lang_data['id_lang']] = '';
            }
    ...
    public function hookDisplayPDFInvoice($params)
    {
        ...
        if ($module_settings['enable'] == 1) {
            ...
            /** Start Changes to add the Payment Fee on Invoice PDF
             * Fetching the fee data from the DB and showing the data if it exists
             * @date 15-10-2024
             * NAOct2024 payment_fee
             * @modifier Nikhil Aggarwal 
             */
            ...
            if (!empty($result_fields_data)) {
                ...
                $firstTPL = $this->display(__FILE__, 'custom_fields_data_content_invoice.tpl');
            }

            $sql = "SELECT * FROM " . _DB_PREFIX_ . "kb_supercheckout_payment_fee WHERE id_order = " . (int) $id_order;
            ...
            if (isset($result) && !empty($result) && isset($result['paymentfee_applicable']) && $result['paymentfee_applicable'] != 0) {
                ...
                /**
                 * Change done to fix the issue with the payment fee on the checkout forthe online payment methods
                 * @date 12/12/2025
                 * 
                 */
                // FIX: PrestaShop does not send $params['order'] in displayPDFInvoice, only the invoice object.
                // We use the id_order to load the order and get its currency.}
                $orderObj = null;

                // Try to retrieve the order from $params['order'] (if it exists)
                if (isset($params['order']) && $params['order'] instanceof Order) {
                    $orderObj = $params['order'];
                } else {
                    // Correct approach: use the invoice object to obtain the id_order
                    if (isset($params['object']) && isset($params['object']->id_order)) {
                        $orderObj = new Order((int) $params['object']->id_order);
                    }
                }

                // Get currency ID safely
                $currencyId = ($orderObj && $orderObj->id_currency)
                    ? (int) $orderObj->id_currency
                    : (int) $this->context->currency->id;

                $priceFormatter = new PriceFormatter();
                $this->context->smarty->assign('fee', $priceFormatter->format($fee, $currencyId));

                $secondTPL = $this->display(__FILE__, 'kbfee_invoice.tpl');

            }
            return $firstTPL . $secondTPL;
            // Changes end by Nikhil
        }
    }
    ...
    public function hookActionEmailAddAfterContent(&$params)
    {
        ...
        if ($params['template'] == 'order_conf') { // Let's edit content of Order's Confirmation email
             ...
             * @date 04-07-2025
             */
            $order_id = Db::getInstance()->getValue('SELECT id_order FROM ' . _DB_PREFIX_ . 'orders WHERE id_cart = ' . (int) $cart_id);
            ...
            /** Start Changes to add the Payment Fee on the Order confirmation Email
             * Showing the Payment Fee on the Order Confirmation Email
             * @date 15-10-2024
             * NAOct2024 payment_fee
             * @modifier Nikhil Aggarwal 
             */
        ...
        WHERE id_order = " . (int) $order_id;
            ...
            $fee = Db::getInstance()->getValue($sql);

            if ($fee > 0) {
                $priceFormatter = new PriceFormatter();
                $payment_fee = $priceFormatter->format($fee, $this->context->currency->id);

                // Display the fee in the email without altering totals
                $content = $this->l('Payment Method Fee') . ": " . $payment_fee;

                // Insert it at the end of the email
                $params['template_html'] .= $content;
            }
            // Changes end by Nikhil
        }
    }
    ...
    public function hookActionValidateOrder($params)
    {
        ...
        /** Start Changes to add the Payment Fee on Checkout
         * Updating the prices in the orders table and adding the payment fee in the order
         * @date 15-10-2024
         * NAOct2024 payment_fee
         * @modifier Nikhil Aggarwal 
         */
        $query1 = "SELECT * FROM " . _DB_PREFIX_ . "orders WHERE id_order = '" . (int) $id_order . "'";
        $result1 = Db::getInstance()->getRow($query1);

        if (!empty($result1)) {
            $sql2 = "SELECT * FROM " . _DB_PREFIX_ . "kb_supercheckout_payment_fee WHERE id_cart = '" . (int) $id_cart . "'";
            $result2 = Db::getInstance()->getRow($sql2);
            if (!empty($result2)) {
                $fee = (float) $result2['paymentfee_applicable'];
                // $params['order']->total_paid = $fee + (float) $result1['total_paid'];
                // $params['order']->total_paid_tax_incl = $fee + (float) $result1['total_paid_tax_incl'];
                // $params['order']->total_paid_tax_excl = $fee + (float) $result1['total_paid_tax_excl'];
                // $params['order']->total_products = $fee + (float) $result1['total_products'];
                // $params['order']->total_products_wt = $fee + (float) $result1['total_products_wt'];

                $payment_method = $params['order']->module;

                $module_id = Db::getInstance()->getValue('SELECT id_module FROM ' . _DB_PREFIX_ . 'module WHERE name = "' . pSQL($payment_method) . '"');
                if ($fee > 0) {
                    Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'kb_supercheckout_payment_fee SET id_order = ' . (int) $id_order . ', id_module = ' . pSQL($module_id) . ' WHERE id_cart = ' . (int) $id_cart);
                }
            }
        }
        // Changes end by Nikhil
    ...
    public function hookDisplayOrderDetail()
    {
        ...
        if ($module_settings['enable'] == 1) {
            ...
            $this->smarty->assign('empty', $empty);
            /** Start Changes to add the Payment Fee on Checkout
             * Showing the fee on the order page on front
             * @date 15-10-2024
             * NAOct2024 payment_fee
             * @modifier Nikhil Aggarwal 
             */
            $firstTPL = $this->display(__FILE__, 'custom_fields_data_on_order_history.tpl');
            ...
            if (isset($result) && !empty($result) && isset($result['paymentfee_applicable']) && $result['paymentfee_applicable'] != 0) {
                ...
                $priceFormatter = new PriceFormatter();
                $this->context->smarty->assign('fee', $priceFormatter->format($fee, (int) $order->id_currency));
                $secondTPL = $this->display(__FILE__, 'kbfee_orderhistory.tpl');
            }
            return $firstTPL . $secondTPL;
            // Changes end by Nikhil
        }
    ...
    public function getDefaultSettings()
    {
        $settings = array(
            ...
            /**
             * Changes for default values warning popup of sendinblue and klaviyo form
             * @author Prvind Panday
             * @date 08 March 2024
             */
            'SendinBlue' => array('enable' => 0, 'api' => '', 'list' => '', 'default' => 0),
            'klaviyo' => array('enable' => 0, 'api' => '', 'list' => '', 'default' => 0),
            ...
            /** Start Changes to add the Payment Fee on Checkout
             * Setting the default value for the payment fee method
             * @date 07-10-2024
             * NAOct2024 payment_fee
             * @modifier Nikhil Aggarwal */
            'paymentfee_method' => Cart::ONLY_PRODUCTS,
            'paymenttax_method' => 'inc_tax',
            // Changes end by Nikhil
     ...
     * Add this hook before the #placeorderButton in frontend SuperCheckout tpl file
    ...
    /**
     * If the current controller is "supercheckout", function will overrides it with "order". Modifying the $_GET["controller"] variable to change the controller being executed to 'order'
     * @author Ashish Kumar
     * @date 06-05-2024
     * AKMay2024 stripe-checkout-compatible
     */
    ...
    {
        if (!empty($_GET["controller"]) && $_GET["controller"] == "supercheckout") { // Message for Addons Team: Use of the $_GET is required for working of the module as needed to pass override GET data to Prestashop core.
            $_GET["controller"] = "order";   // Message for Addons Team: Use of the $_GET is required for working of the module as needed to pass override GET data to Prestashop core.
        }
    }
    ...
    public function deleteEmailFromMarketingServices($email)
    {
        ...
        if ($config['email_marketing_delete'] == 1) {
            try {
                ...
                $Mailchimp = new MailChimpKbSC($api_key);
    ...
    /** Start Changes to add the Payment Fee on Checkout
     * Below hook is responsible to add a column on the order page in the admin panel to show the payment fee
     * @date 15-10-2024
     * NAOct2024 payment_fee
     * @modifier Nikhil Aggarwal 
     */
    public function hookDisplayAdminOrder($params)
    {
        ...
        if (isset($module_settings) && $module_settings['enable'] == 1) {
            ...
            if (isset($result) && !empty($result) && isset($result['paymentfee_applicable']) && $result['paymentfee_applicable'] != 0) {
                ...
                $priceFormatter = new PriceFormatter();
                $this->context->smarty->assign('fee', $priceFormatter->format($fee, (int) $order->id_currency));
                return $this->display(__FILE__, 'kbpaymentfee_adminorders.tpl');
            }
        }
        return '';
    }
