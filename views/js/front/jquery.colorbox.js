...
(function ($, document, window) {
  ...
  function setSize(size, dimension) {
    return Math.round(
      (/%/.test(size)
        ? (dimension === "x" ? $window.width() : $window.height()) / 100
        : 1) * parseInt(size, 10)
    );
  }
  ...
  publicMethod.position = function (speed, loadedCallback) {
    ...
    if (settings.bottom !== false) {
      ...
      top += Math.round(
        Math.max(
          $window.height() - settings.h - loadedHeight - interfaceHeight,
          0
        ) / 2
      );
    }
  ...
  publicMethod.load = function (launched) {
    ...
    if (settings.inline) {
        ...
        .load(function () {
          ...
          if (settings.h) {
            photo.style.marginTop =
              Math.max(settings.h - photo.height, 0) / 2 + "px";
          }
      ...
      $loadingBay.load(href, settings.data, function (data, status, xhr) {
        prep(
          status === "error"
            ? $tag(div, "Error").text("Request unsuccessful: " + xhr.statusText)
            : $(this).contents()
        );
      });
    }
