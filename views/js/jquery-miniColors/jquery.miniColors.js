  ...
  (function ($) {
    ...
    function init(input) {
      var minicolors = $('<span class="minicolors" />'),
        sliderType =
          input.attr("data-slider") || $.minicolors.settings.defaultSlider;
      ...
      minicolors
        ...
        .toggleClass(
          "minicolors-swatch-left",
          input.attr("data-swatch-position") === "left"
        )
        .toggleClass(
          "minicolors-with-opacity",
          input.attr("data-opacity") !== undefined
        );
      ...
      // Disable textfield
      if (input.attr("data-textfield") === "false")
        input.addClass("minicolors-hidden");

      // Inline controls
      if (input.attr("data-control") === "inline")
    ...
    function show(input) {
      ...
      panel.stop(true, true).fadeIn($.minicolors.settings.showSpeed);
    }
    ...
    function hide() {
      $(".minicolors:not(.minicolors-inline)").each(function () {
        ...
        minicolors
          .find(".minicolors-panel")
          .fadeOut($.minicolors.settings.hideSpeed, function () {
    ...
    function updateFromControl(input) {
      ...
      switch (sliderType) {
        ...
        default:
          // Calculate hue, saturation, and brightness
          hue = keepWithin(
            360 - parseInt(sliderPos.y * (360 / slider.height())),
            0,
            360
          );
          saturation = keepWithin(
      ...
      if (hex + opacity !== input.data("last-change")) {
        input.data("last-change", hex + opacity).trigger("change", input);
      }
    }
      ...
      if (hasOpacity) {
        opacity =
          input.attr("data-opacity") === ""
            ? 1
            : keepWithin(
                parseFloat(input.attr("data-opacity")).toFixed(2),
                0,
                1
              );
    ...
    function convertCase(string) {
      return $.minicolors.settings.letterCase === "uppercase"
        ? string.toUpperCase()
        : string.toLowerCase();
    }
          ...
          input.val(
            convertCase(
              parseHex(
                input.val() !== ""
                  ? input.val()
                  : convertCase(parseHex(input.attr("data-default"), true)),
                true
              )
            )
          );
