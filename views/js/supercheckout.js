...
$(document).ready(function () {
  $("#supercheckout_configuration_form table tr td").each(function () {
    ...
    for (let ab in data) {
      if (
        data[ab].name == "data-toggle" ||
        data[ab].name == "data-placement" ||
        data[ab].name == "data-original-title"
      ) {
   ...
   * Added the condition to show/hide the payment fee fields based on selected payment fee type
   * @date 08-10-2024
   * NAOct2024 payment_fee
   * @modifier Nikhil Aggarwal
   */
  function toggleFeeFields(moduleId, selectedType) {
    var fixedFeeRow = $("#fixed_fee_row_" + moduleId);
    var percentageFeeRow = $("#percentage_fee_row_" + moduleId);

    // Hide both fields initially
    fixedFeeRow.hide();
    percentageFeeRow.hide();

    // Show relevant fields based on the selected option
    if (selectedType === "fixed") {
      fixedFeeRow.show();
    } else if (selectedType === "percentage") {
      percentageFeeRow.show();
    } else if (selectedType === "fixed_percentage") {
      fixedFeeRow.show();
      percentageFeeRow.show();
    }
  }

  $(".payment_fee_type").each(function () {
    var moduleId = $(this).data("module-id");
    var selectedType = $(this).val();
    toggleFeeFields(moduleId, selectedType);
  });

  // Listen for changes on the select dropdown
  $(".payment_fee_type").change(function () {
    var moduleId = $(this).data("module-id");
    var selectedType = $(this).val();
    toggleFeeFields(moduleId, selectedType);
  });
  ...
  $('input[name="velocity_supercheckout[ip_addresses]"]').keypress(
    function (e) {
      if (e.keyCode == 13) {
        $('input[name="velocity_supercheckout[ip_addresses]"]').val(
          $('input[name="velocity_supercheckout[ip_addresses]"]').val() + ", ",
        );
  ...
  // getklaviyoList();
  /*
   * Changes done by Manish to call the lsiting fucntion only when the api key is not blank
   * @date 10-03-2026
   * @modifier Manish
   * MPMAR2026 Unnecessary_error_issue
   */

  if ($("#supercheckout_mailchimp_key").val().trim() != "") {
    getMailChimpList();
  }
  if ($("#supercheckout_SendinBlue_key").val().trim() != "") {
    getSendinBlueList();
  }
  if ($("#supercheckout_klaviyo_key").val().trim() != "") {
    getklaviyoList();
  }
  ...
  $(".bootbox-design-edit-html").click(function () {
    ...
    if ($("#" + id).length) {
      ...
      var hidden_field =
        ...
        id +
        '" name="velocity_supercheckout[design][html][' +
    ...
    bootbox.confirm(
      "<h4>" +
        $("#modals_bootbox_prompt_header_html").val() +
        '</h4><textarea id="text_area_html_' +
        splitId[splitId.length - 1] +
        '" class="supercheckout_textarea_html" >' +
        stored_value +
        '</textarea><h4 style="color:red;">' +
        double_quotes_warning +
        "</h4>",
      function (result) {
        if (result) {
          html_string = $("#text_area_html_" + splitId[splitId.length - 1])
            .val()
            .replace(/(\r\n|\n|\r)/gm, "<br/>");
          $("#" + id).val(html_string);
        }
  ...
  //2-Column Layout
  $('input[name="velocity_supercheckout[column_width][2_column][1]"]').css(
    "width",
    $('input[name="velocity_supercheckout[column_width][2_column][1]"]').val() +
      "%",
  );
  $('input[name="velocity_supercheckout[column_width][2_column][2]"]').css(
    "width",
    $('input[name="velocity_supercheckout[column_width][2_column][2]"]').val() -
      1 +
      "%",
  );

  //3-Column Layout
  $('input[name="velocity_supercheckout[column_width][3_column][1]"]').css(
    "width",
    $('input[name="velocity_supercheckout[column_width][3_column][1]"]').val() +
      "%",
  );
  $('input[name="velocity_supercheckout[column_width][3_column][2]"]').css(
    "width",
    $('input[name="velocity_supercheckout[column_width][3_column][2]"]').val() +
      "%",
  );
  $('input[name="velocity_supercheckout[column_width][3_column][3]"]').css(
    "width",
    $('input[name="velocity_supercheckout[column_width][3_column][3]"]').val() -
      1 +
      "%",
  );
...
function dialogExtraHtml(e) {
  ...
  bootbox.confirm(
    "<h4>" +
      $("#modals_bootbox_prompt_header_html").val() +
      '</h4><textarea id="text_area_html" class="supercheckout_textarea_html" >' +
      stored_value +
      '</textarea><h4 style="color:red;">' +
      double_quotes_warning +
      "</h4>",
...
/**
 * Start Changes to make the payment fee decimal
 * Function to validate the decimal number
 * NAMar2025 decimal_paymentfee
 * @date 04-03-2025
 * @modifier Nikhil Aggarwal
 */
function isValidPositiveNumber(str) {
  return /^(0|[1-9]\d*)(\.\d+)?$/.test(str); // Allows only positive integers and decimals
}
// Changes end by Nikhil

// Validate Minimum and Maximum Fees
/** Start Changes to add the Payment Fee on Checkout
 * Function to validate the Min and Max fees of the payment method
 * @date 08-10-2024
 * NAOct2024 payment_fee
 * @modifier Nikhil Aggarwal
 */
function validateMinMaxFees(moduleId) {
  var isValidMinMax = true;

  // Clear any previous error messages
  $("#minimum_fee_error_" + moduleId).html("");
  $("#maximum_fee_error_" + moduleId).html("");

  // Check if Payment Fee Status is enabled before applying validation
  var paymentFeeStatus = $(
    'select[name="velocity_supercheckout_payment[payment_method][' +
      moduleId +
      '][paymentfee][status]"]',
  ).val();

  // Only apply validation if Payment Fee Status is enabled (value = "1")
  if (paymentFeeStatus !== "1") {
    return isValidMinMax; // Return true if payment fee is disabled
  }

  // Get the values of the Minimum and Maximum Fee fields
  var minFee = $("#minimum_fee_" + moduleId)
    .val()
    .trim();
  var maxFee = $("#maximum_fee_" + moduleId)
    .val()
    .trim();

  // Validate that they are numbers if not empty
  if (minFee !== "" && !isValidPositiveNumber(minFee)) {
    $("#minimum_fee_error_" + moduleId).html(paymentFeeValidNumError);
    isValidMinMax = false;
  }

  if (maxFee !== "" && !isValidPositiveNumber(maxFee)) {
    $("#maximum_fee_error_" + moduleId).html(paymentFeeValidNumError);
    isValidMinMax = false;
  }

  // If both min and max are not empty and not zero, ensure min is less than max
  if (
    minFee !== "" &&
    maxFee !== "" &&
    parseFloat(minFee) > parseFloat(maxFee) &&
    minFee !== "0" &&
    maxFee !== "0"
  ) {
    $("#maximum_fee_error_" + moduleId).html(paymentFeeMaxError);
    isValidMinMax = false;
  }
...
/** Start Changes to add the Payment Fee on Checkout
 * Function to validate the Fixed and Percentage fees of the payment method
 * @date 08-10-2024
 * NAOct2024 payment_fee
 * @modifier Nikhil Aggarwal
 */
function validateFees(moduleId) {
  var isValid = true;

  // Clear existing error messages
  $("#fixed_fee_error_" + moduleId).html("");
  $("#percentage_fee_error_" + moduleId).html("");
  ...
  // Only apply validation if Payment Fee Status is enabled (value = "1")
  if (paymentFeeStatus !== "1") {
    return isValid; // Return true if payment fee is disabled
  }
  ...
  // Validate Fixed Fee if 'fixed' or 'fixed_percentage' is selected
  if (selectedType === "fixed" || selectedType === "fixed_percentage") {
    var fixedFee = $("#fixed_fee_" + moduleId)
      .val()
      .trim();
    if (fixedFee === "" || !isValidPositiveNumber(fixedFee)) {
      $("#fixed_fee_error_" + moduleId).html(paymentMethodFeeError);
      isValid = false;
    } else {
      $("#fixed_fee_error_" + moduleId).html("");
    }
  ...
  // Validate Percentage Fee if 'percentage' or 'fixed_percentage' is selected
  if (selectedType === "percentage" || selectedType === "fixed_percentage") {
    var percentageFee = $("#percentage_fee_" + moduleId)
      .val()
      .trim();
    if (
      percentageFee === "" ||
      ...
      $("#percentage_fee_error_" + moduleId).html(paymentMethodFeePercentError);
      isValid = false;
    } else {
      $("#percentage_fee_error_" + moduleId).html("");
    }
...
function validate_data() {
  ...
  $("#paypal_client_id_error").html("");
  $("#paypal_client_secret_error").html("");
  ...
  var ip_values = $('input[name="velocity_supercheckout[ip_addresses]"]')
    .val()
    .trim();
  if (ip_values != "") {
  ...
  var check_free_shipping_value = velovalidation.checkAmount(
    $('input[name="velocity_supercheckout[free_shipping_amount]"]'),
  );
  if (check_free_shipping_value != true) {
    general_settings_error = true;
    $("#free_shipping_amount_error").html(check_free_shipping_value);
  } else if (
    $('input[name="velocity_supercheckout[free_shipping_amount]"]').val()
      .length > 10
  ) {
    general_settings_error = true;
  ...
  /* Start Code Added by Priyanshu on 3-June-2020 for Paypal Login validation */

  if ($("#supercheckout_paypal_login").is(":checked")) {
    if ($("#velocity_supercheckout_paypal_client_id").val().trim() == "") {
      login_error = true;
      $("#paypal_client_id_error").html(paypal_client_id_error);
    }
    if ($("#velocity_supercheckout_paypal_client_secret").val().trim() == "") {
      login_error = true;
      $("#paypal_client_secret_error").html(paypal_client_secret_error);
    }
  }

  /* End of Code Added by Priyanshu on 3-June-2020 for Paypal Login validation */
  ...
  if (
    $(
      "input[name='velocity_supercheckout[payment_method][display_style]']:checked",
    ).val() == 1 ||
    $(
      "input[name='velocity_supercheckout[payment_method][display_style]']:checked",
    ).val() == 2
  ) {
    $("input[id^=velocity_supercheckout_payment_method_logo_width_]").each(
      function () {
        ...
        if ($(this).val().trim() == "") {
          ...
          $(
            "#payment_method_logo_width_error_" +
              field_id_arr[field_id_arr.length - 1],
          ).html(empty_width_error);
        } else if (
          !isNormalInteger($(this).val().trim()) &&
          $(this).val().trim() != "auto"
        ) {
          payment_method_error = true;
          $(
    ...
    $("input[id^=velocity_supercheckout_payment_method_logo_height_]").each(
      function () {
        field_id = $(this).attr("id");
        field_id_arr = field_id.split("_");
        if ($(this).val().trim() == "") {
          payment_method_error = true;
          $(
            "#payment_method_logo_height_error_" +
              field_id_arr[field_id_arr.length - 1],
          ).html(empty_height_error);
        } else if (
          !isNormalInteger($(this).val().trim()) &&
          $(this).val().trim() != "auto"
        ) {
  ...
  /** Start Changes to add the Payment Fee on Checkout
   * Added the validation for the payment method fee fields when the form is submitted
   * @date 08-10-2024
   * NAOct2024 payment_fee
   * @modifier Nikhil Aggarwal
   */
  ...
  $(".payment_fee_type").each(function () {
    var moduleId = $(this).data("module-id");
    var isValidFixedPercentage = validateFees(moduleId);
    var isValidMinMax = validateMinMaxFees(moduleId);
    allValid = isValidMinMax && isValidFixedPercentage && allValid; // If any module is invalid, prevent submission
  });
  // Changes end by Nikhil

  if (
    $(
      "input[name='velocity_supercheckout[shipping_method][display_style]']:checked",
    ).val() == 1 ||
    $(
      "input[name='velocity_supercheckout[shipping_method][display_style]']:checked",
    ).val() == 2
  ) {
    $("input[id^=velocity_supercheckout_delivery_method_logo_width_]").each(
      function () {
        field_id = $(this).attr("id");
        field_id_arr = field_id.split("_");
        if ($(this).val().trim() == "") {
          ...
          $(
            "#delivery_method_logo_width_error_" +
              field_id_arr[field_id_arr.length - 1],
          ).html(empty_width_error);
        } else if (
          !isNormalInteger($(this).val().trim()) &&
    ...
    $("input[id^=velocity_supercheckout_delivery_method_logo_height_]").each(
      function () {
        field_id = $(this).attr("id");
        field_id_arr = field_id.split("_");
        if ($(this).val().trim() == "") {
          delivery_method_error = true;
          $(
            "#delivery_method_logo_height_error_" +
              field_id_arr[field_id_arr.length - 1],
          ).html(empty_height_error);
        } else if (
          !isNormalInteger($(this).val().trim()) &&
  ...
  if (
    $("input[name='velocity_supercheckout[cart_image_size][width]']")
      .val()
      .trim() == ""
  ) {
    cart_error = true;
    ...
    $("#cart_product_image_size_width_error").html(
      valid_width_error_product_image,
    );
  } else {
    $("#cart_product_image_size_width_error").html("");
  }
  ...
  if (
    $("input[name='velocity_supercheckout[cart_image_size][height]']")
      .val()
      .trim() == ""
  ) {
    cart_error = true;
    ...
    $("#cart_product_image_size_height_error").html(
      valid_height_error_product_image,
    );
  } else {
    $("#cart_product_image_size_height_error").html("");
  }
  ...
  /** Start Changes to add the Payment Fee on Checkout
   * Added the condition to payment fee form validation also along with the payment method form validation
   * @date 08-10-2024
   * NAOct2024 payment_fee
   * @modifier Nikhil Aggarwal
   */
  if (
    ...
    if (payment_method_error || !allValid) {
    ...
    var errorHtml =
      '<div class="bootstrap supercheckout-message"><div class="alert alert-danger">';
    errorHtml +=
    ...
    setTimeout(function () {
      $("#velsof_supercheckout_container .supercheckout-message").remove();
    }, 5000);
  ...
  $.ajax({
    ...
    success: function (json) {
      ...
      if (json == "false")
        ...
        if (ps_ver == 15) html += 'class="selectpicker vss_sc_ver15"';
...
function getSendinBlueList() {
  ...
  if (key != "") {
    $.ajax({
      ...
      beforeSend: function () {
        // $("#supercheckout_list").html("");
        //Changes done by Manish to remove listings related to the same email marketing tool
        $("#SendinBlue_supercheckout_list").html("");
  ...
  if (key != "") {
    $.ajax({
      ...
      beforeSend: function () {
        // $("#supercheckout_list").html("");
        //Changes done by Manish to remove listings related to the same email marketing tool
        $("#klaviyo_supercheckout_list").html("");
  ...
  if (confirm(remove_cnfrm_msg) == true) {
    $.ajax({
      ...
      success: function (json) {
        ...
        $("#payment-img-" + id).attr(
          "src",
          module_path + "views/img/admin/no-image.jpg",
        );
      },
    });
  }
...
function addNewCustomFieldPopup() {
  ...
  $("#modal_add_new_custom_field_form select option:selected").prop(
    "selected",
    false,
  );
  $("#modal_add_new_custom_field_form select option:first").prop(
    "selected",
    "selected",
  );
  $("#modal_add_new_custom_field_form").modal({
    show: true,
    backdrop: "static",
  });
}

function addNewCustomerProfilePopup() {
  $('#modal_add_new_customer_profile_form :input[type="text"]').val("");
  $("#modal_add_new_customer_profile_form textarea").val("");
  $("#modal_add_new_customer_profile_form select option:selected").prop(
    "selected",
    false,
  );
  $("#modal_add_new_customer_profile_form select option:first").prop(
    "selected",
    "selected",
  );
  $("#modal_add_new_customer_profile_form").modal({
    show: true,
    backdrop: "static",
  });
}
...
function changeLanguageBox(objE, elementToChange) {
  ...
  $("#" + elementToChange + "_language_" + idLanguage).removeClass(
    "hidden_custom",
  );
}
...
function checkFieldType(objE) {
  ...
  if (
    ...
    $('select[name="custom_fields[validation_type]"]').prop(
      "disabled",
      "disabled",
    );
  } else {
...
function checkFieldTypeEdit(objE) {
  ...
  if (
    ...
    $('select[name="edit_custom_fields[validation_type]"]').prop(
      "disabled",
      "disabled",
    );
  } else {
...
function displayEditCustomFieldPopup(idCustomField) {
  ...
  $.ajax({
    ...
    url: scp_ajax_action,
    data:
      $("#supercheckout_configuration_form").serialize() +
      "&ajax=true&custom_fields_action=displayEditCustomFieldForm&id=" +
      idCustomField,
    async: false,
    dataType: "json",
    success: function (json) {
      $("#modal_edit_custom_field_form").html(json.response);
      $("#modal_edit_custom_field_form").modal({
        show: true,
        backdrop: "static",
      });
    },
...
function displayEditCustomerProfilePopup(idProfile) {
  //    $('#modal_edit_custom_field_form').html("");
  // Send ajax request to save the data
  $.ajax({
    type: "POST",
    ...
    data:
      $("#supercheckout_configuration_form").serialize() +
      "&ajax=true&customer_profile_action=displayEditCustomerProfileForm&id_profile=" +
      idProfile,
    async: false,
    dataType: "json",
    success: function (json) {
      $("#modal_edit_customer_profile_form").html(json.response);
      /*
       * Prevent errors when jQuery Uniform plugin is not available on newer PrestaShop versions
       * 26-02-2026
       */
      if ($.fn && $.fn.uniform) {
        $("#modal_edit_customer_profile_form input[type='checkbox']").uniform();
      }
      $("#modal_edit_customer_profile_form").modal({
        show: true,
        backdrop: "static",
      });
    },
...
function submitEditForm() {
  ...
  if (errorOccured != 0) {
    ...
    $.ajax({
      ...
      data:
        $("#supercheckout_configuration_form").serialize() +
        "&ajax=true&custom_fields_action=editCustomFieldForm",
      async: true,
      dataType: "json",
      beforeSend: function () {
      ...
      success: function (json) {
        ...
        var findRowCount = $(
          "#tr_pure_table_" +
            json.response.id_velsof_supercheckout_custom_fields,
        )
          .children("td:first")
          .text();
        var tableRow =
          '<tr class="row_changed" id="tr_pure_table_' +
          json.response.id_velsof_supercheckout_custom_fields +
          '">';
        tableRow += "<td>" + findRowCount + "</td>";
        tableRow +=
          '<td class="width_25"><div class="div_250px_ellipsis">' +
          json.response.field_label +
          "</div></td>";
        /* Start - Code Added by Raghu on 22-Aug-2017 for fixing 'Custom Fields type translations' issue */
        tableRow +=
          "<td>" +
          getCustomFieldsTypeTranslatedText(json.response.type) +
          "</td>";
        ...
        tableRow +=
          "<td>" +
          getCustomFieldsBlockTranslatedText(json.response.position) +
          "</td>";
        ...
        tableRow +=
          '<a style="margin-top: -26px;" href="javascript://" onclick="displayEditCustomFieldPopup(' +
          json.response.id_velsof_supercheckout_custom_fields +
          ')" type="11" class="velsof-glyphicons2 glyphicons pencil"><i data-toggle="tooltip" data-placement="top" data-original-title="Edit this custom field"></i></a>';
        tableRow +=
          '<a style="margin-top: -26px;" href="javascript://" onclick="deleteCustomFieldRow(' +
          json.response.id_velsof_supercheckout_custom_fields +
          ')" type="11" class="velsof-glyphicons2 glyphicons bin" onclick=""><i data-toggle="tooltip" data-placement="top" data-original-title="Delete this custom field."></i></a>';
        tableRow += "</td>";
        tableRow += "</tr>";
        ...
        $("#loader_edit_form").addClass("hidden_custom");
        setTimeout(function () {
          $("#tbody_custom_fields_data")
            .children()
            .removeClass("row_changed", 1000);
        }, 5000);
      },
    });
...
function submitForm() {
  ...
  if (errorOccured != 0) {
    ...
    $.ajax({
      ...
      data:
        $("#supercheckout_configuration_form").serialize() +
        "&ajax=true&custom_fields_action=addCustomFieldForm",
      async: true,
      dataType: "json",
      beforeSend: function () {
      ...
      success: function (json) {
        ...
        var tableRow =
          '<tr class="pure-table-striped row_changed" id="tr_pure_table_' +
          json.response.id_velsof_supercheckout_custom_fields +
          '">';
...
function submitCustomerProfileForm() {
  $("#loader_add_form_customer_profile").removeClass("hidden_custom");
  var errorOccured = 0;

  var elemLabelBoxes = $(".supercheckout_customer_profiles");
  ...
  if (errorOccured) {
    $("#error_message_profile_name").html(profileNameError);
    $("#error_message_profile_name").removeClass("hidden_custom");
    $("#loader_add_form_customer_profile").addClass("hidden_custom");
    return false;
  } else {
    // Showing the ajax loader
    ...
    $.ajax({
      ...
      data:
        $("#supercheckout_configuration_form").serialize() +
        "&ajax=true&customer_profile_action=addCustomerProfileForm",
      async: true,
      dataType: "json",
      beforeSend: function () {
        closeModalPopup("modal_add_new_customer_profile_form");
      },
      success: function (json) {
        // Adding a row in table
        if (json.response) {
          ...
          var rowCount = $("#table_customer_profile_data > tbody > tr").length;

          var tableRow =
            '<tr class="pure-table-striped row_changed" id="tr_pure_table_' +
            json.id_profile +
            '">';
          tableRow += '<td class="center">' + rowCount + "</td>";
          tableRow +=
            '<td class="width_25 center"><div class="div_250px_ellipsis">' +
            json.field_label +
            "</div></td>";
          tableRow += '<td class="center">' + activeText + "</td>";
          tableRow += '<td class="center" style="padding: 12px;">';
          tableRow +=
            '<a style="margin-top: -26px;" href="javascript://" onclick="displayEditCustomerProfilePopup(' +
            json.id_profile +
            ')" type="11" class="velsof-glyphicons2 glyphicons pencil"><i data-toggle="tooltip" data-placement="top" data-original-title="Edit this custom field"></i></a>';
          tableRow +=
            '<a style="margin-top: -26px;" href="javascript://" onclick="deleteCustomerProfileRow(' +
            json.id_profile +
            ')" type="11" class="velsof-glyphicons2 glyphicons bin" onclick=""><i data-toggle="tooltip" data-placement="top" data-original-title="Delete this custom field."></i></a>';
          tableRow += "</td>";
          tableRow += "</tr>";
          ...
          // Removing the success green color from all the previous edited/added rows
          $("#table_customer_profile_data")
            .children()
            .removeClass("row_changed");
          $("#tr_customer_profile_add_new").before(tableRow);
          $("#div_custom_fields_success").removeClass("hidden_custom");
          $("#loader_add_form_customer_profile").addClass("hidden_custom");
          setTimeout(function () {
            $("#table_customer_profile_data")
...
function submitEditCustomerProfileForm() {
  $("#loader_edit_form_customer_profile").removeClass("hidden_custom");
  var errorOccured = 0;

  var elemLabelBoxes = $(".supercheckout_edit_customer_profiles");
  elemLabelBoxes.each(function (index) {
    if ($(this).val() == "") {
      errorOccured = 1;
    }
  ...
  if (errorOccured) {
    $("#error_message_edit_profile_name").html(profileNameError);
    $("#error_message_edit_profile_name").removeClass("hidden_custom");
    $("#loader_edit_form_customer_profile").addClass("hidden_custom");
    return false;
  } else {
    // Showing the ajax loader
    ...
    $.ajax({
      ...
      data:
        $("#supercheckout_configuration_form").serialize() +
        "&ajax=true&customer_profile_action=editCustomerProfileForm",
      async: true,
      dataType: "json",
      beforeSend: function () {
        closeModalPopup("modal_edit_customer_profile_form");
      },
      success: function (json) {
        // Adding a row in table
        if (json.response) {
          ...
          var findRowCount = $("#tr_pure_table_" + json.id_profile)
            .children("td:first")
            .text();
          var tableRow =
            '<tr class="row_changed" id="tr_pure_table_' +
            json.id_profile +
            '">';
          tableRow += '<td class="center">' + findRowCount + "</td>";
          tableRow +=
            '<td class="width_25 center"><div class="div_250px_ellipsis">' +
            json.field_label +
            "</div></td>";
          ...
          // Removing the success green color from all the previous edited/added rows
          $("#tbody_customer_profile_data")
            .children()
            .removeClass("row_changed");
          $("#tr_pure_table_" + json.id_profile).replaceWith(tableRow);
          $("#div_custom_fields_success").removeClass("hidden_custom");
          $("#loader_edit_form_customer_profile").addClass("hidden_custom");
          setTimeout(function () {
            $("#tbody_customer_profile_data")
...
function validateForm() {
  ...
  if (
    elemType.val() == "selectbox" ||
    elemType.val() == "radio" ||
    elemType.val() == "checkbox"
  ) {
          ...
          if (alphanumeric.length != 2) {
            ...
            for (var j = 0; j < alphanumeric.length; j++) {
              if (j == 0) {
                ...
                /**
                 * Start Changes to fix the issue of data not being saved for Persian Language
                 * Replacing expression from /^[a-zA-Z0-9]+$/ to /^[a-zA-Z0-9\u0600-\u06FF\s]+$/
                 * NASep2023 custom_persian
                 * @date 26-09-2023
                 * @modifier Nikhil Aggarwal
                 */
                var expression = /^[a-zA-Z0-9\u0600-\u06FF\s]+$/;
                // Changes end by Nikhil
              } else {
                /**
                 * Start Changes to fix the issue of data not being saved for Persian Language
                 * Replacing expression from /^[a-zA-Z0-9 -_/]+$/ to /^[\u0600-\u06FFa-zA-Z0-9 -_/]+$/
                 * NASep2023 custom_persian
                 * @date 26-09-2023
                 * @modifier Nikhil Aggarwal
                 */
                var expression = /^[\u0600-\u06FFa-zA-Z0-9 -_/]+$/;
                // Changes end by Nikhil
              }
...
function deleteCustomFieldRow(idCustomField) {
  ...
  if (canDelete == true) {
    ...
    $.ajax({
      ...
      url: scp_ajax_action,
      data:
        "&ajax=true&custom_fields_action=deleteCustomFieldRow&id_velsof_supercheckout_custom_fields=" +
        idCustomField,
      async: false,
      dataType: "json",
      success: function (json) {
...
function deleteCustomerProfileRow(idCustomerProfile) {
  var canDelete = confirm(areYouSureToDelete);
  if (canDelete == true) {
    // Send ajax request to save the data
    $.ajax({
      ...
      url: scp_ajax_action,
      data:
        "&ajax=true&customer_profile_action=deleteCustomerProfileRow&id_profile=" +
        idCustomerProfile,
      async: false,
      dataType: "json",
      success: function (json) {
        $("#tr_pure_table_" + idCustomerProfile).addClass("hidden_custom");
        $("#div_custom_fields_success").removeClass("hidden_custom");
      },
    });
  }
...
function submitGDPRSettingForm() {
  ...
  if (errorOccured != 0) {
    ...
    $.ajax({
      ...
      url: scp_ajax_action,
      data:
        $("#modal_add_new_gdpr_privacy_form :input").serialize() +
        "&ajax=true&gdpr_privacy_action=addNewPrivacyPolicy",
      async: true,
      dataType: "json",
      beforeSend: function () {
      ...
      success: function (json) {
        ...
        var tableRow =
          '<tr class="pure-table-striped row_changed" id="tr_policy_table_' +
          json.response.policy_id +
          '">';
        tableRow += "<td>" + rowCount + "</td>";
        tableRow +=
          '<td class="width_25"><div class="div_250px_ellipsis">' +
        ...
        tableRow += '<td class="center" style="padding: 12px;">';
        tableRow +=
          '<div style="display:inline-block;float:left;"><a style="padding:5px" href="javascript://" onclick="displayEditGDPRPolicyPopup(' +
          json.response.policy_id +
          ')" type="11" class="velsof-glyphicons2 glyphicons pencil"><i data-toggle="tooltip" data-placement="top" data-original-title="Edit this policy"></i></a></div>';
        tableRow +=
          '<div style="display:inline-block;"><a style="padding:5px" href="javascript://" onclick="deleteGDPRPolicyRow(' +
          json.response.policy_id +
          ')" type="11" class="velsof-glyphicons2 glyphicons bin" onclick=""><i data-toggle="tooltip" data-placement="top" data-original-title="Delete this policy."></i></a></div>';
        tableRow += "</td>";
        tableRow += "</tr>";
...
function deleteGDPRPolicyRow(policy_id) {
  ...
  if (canDelete == true) {
    ...
    $.ajax({
      ...
      url: scp_ajax_action,
      data:
        "&ajax=true&gdpr_privacy_action=deletePrivacyPolicyRow&policy_id=" +
        policy_id,
      async: true,
      dataType: "json",
      success: function (json) {
...
function displayEditGDPRPolicyPopup(policy_id) {
  ...
  $.ajax({
    ...
    url: scp_ajax_action,
    data:
      "ajax=true&gdpr_privacy_action=displayEditGDPRPolicyForm&id=" + policy_id,
    async: true,
    dataType: "json",
    success: function (json) {
      $("#modal_edit_gdpr_policy_form").html(json.response);
      $("#modal_edit_gdpr_policy_form").modal({
        show: true,
        backdrop: "static",
      });
      $("body").removeClass("loading");
...
function submitEditGDPRSettingForm() {
  ...
  if (errorOccured != 0) {
    ...
    $.ajax({
      ...
      url: scp_ajax_action,
      data:
        $("#modal_edit_gdpr_policy_form :input").serialize() +
        "&ajax=true&gdpr_privacy_action=editPrivacyPolicyForm",
      async: true,
      dataType: "json",
      beforeSend: function () {
        $("body").addClass("loading");
      ...
      success: function (json) {
        ...
        var findRowCount = $("#tr_policy_table_" + json.response.policy_id)
          .children("td:first")
          .text();
        var tableRow =
          '<tr class="row_changed" id="tr_policy_table_' +
          json.response.policy_id +
          '">';
        tableRow += "<td>" + findRowCount + "</td>";
        tableRow +=
...
function getAcceptedCustomerPolicy() {
  ...
  if (searchData != "") {
    $("#error_message_supercheckout_gdpr_filter")
      .removeClass("hidden_custom")
      .addClass("hidden_custom");
    $("body").addClass("loading");
    $("#rm_loader").show();
    $.ajax({
      type: "POST",
      ...
      data: {
        ajax: true,
        gdpr_privacy_action: "displayFilteredGDPRCustomerData",
...
function resetCustomerPolicy() {
  $("#error_message_supercheckout_gdpr_filter")
    ...
    .addClass("hidden_custom");
  var empty_html =
    '<div class="rm_no_data"><span>' + noDataFound + "</span></div>";
