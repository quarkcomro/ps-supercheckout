...
function updatePrimaryColor(hex, attach, charts) {
  ...
  if (attach === true) attachStylesheet();

  if (charts === true) updateCharts();
  ...
  else themerCustom[themerSelectedTheme].primaryColor = null;
...
function toggleGetCode() {
  ...
  if (
    themerSelectedTheme != 0 ||
    (themerSelectedTheme == 0 && tcs.primaryColor != null)
  ) {
...
var themerAdvanced =
  $.cookie("themerAdvanced") != null
    ? $.cookie("themerAdvanced") == true
    : false;
function themerAdvancedToggle() {
  ...
  if ($("#themer-advanced-toggle").is(":checked")) {
    ...
    $.each(cp, function (k, v) {
      v.attr("data-textfield", true).removeClass("minicolors-hidden");
    });
  } else {
    $("#themer").removeClass("themer-advanced");
    $.each(cp, function (k, v) {
      v.attr("data-textfield", false).addClass("minicolors-hidden");
    });
  }
}
...
function updateCharts() {
  if (typeof primaryColor != "undefined") primaryColor = themerPrimaryColor;

  if (typeof genSparklines != "undefined") genSparklines();

  if (typeof charts == "undefined") return false;
  ...
  if (
    typeof charts.website_traffic_graph != "undefined" &&
    charts.website_traffic_graph.plot != null
  )
    charts.website_traffic_graph.init();

  if (
    typeof charts.website_traffic_overview != "undefined" &&
    charts.website_traffic_overview.plot != null
  )
    charts.website_traffic_overview.init();

  if (
    typeof charts.traffic_sources_pie != "undefined" &&
    charts.traffic_sources_pie.plot != null
  )
    charts.traffic_sources_pie.init();

  if (
    typeof charts.chart_simple != "undefined" &&
    charts.chart_simple.plot != null
  )
    charts.chart_simple.init();

  if (
    typeof charts.chart_lines_fill_nopoints != "undefined" &&
    charts.chart_lines_fill_nopoints.plot != null
  )
    charts.chart_lines_fill_nopoints.init();

  if (
    typeof charts.chart_ordered_bars != "undefined" &&
    charts.chart_ordered_bars.plot != null
  )
    charts.chart_ordered_bars.init();

  if (
    typeof charts.chart_donut != "undefined" &&
    charts.chart_donut.plot != null
  )
    charts.chart_donut.init();

  if (
    typeof charts.chart_stacked_bars != "undefined" &&
    charts.chart_stacked_bars.plot != null
  )
  ...
  if (
    typeof charts.chart_horizontal_bars != "undefined" &&
    charts.chart_horizontal_bars.plot != null
  )
...
function updateTheme(themeSelect) {
  if ($("#themer-theme").val() != themeSelect)
    $("#themer-theme").val(themeSelect);

  themerSelectedTheme = themeSelect; // index
  $.cookie("themerSelectedTheme", themerSelectedTheme);

  var uPrimaryColor =
    themerCustom[themeSelect].primaryColor != null
      ? themerCustom[themeSelect].primaryColor
      : themerThemes[themeSelect].primaryColor;
  ...
  else attachStylesheet();
}

function themerGetCode(less) {
  var tlc;
  if (less === true) tlc = latestCode.less;
  else tlc = latestCode.css();

  //bootbox.alert($('<textarea class="input-block-level" rows="10"></textarea>').val(tlc));
  bootbox.alert(
    $('<pre class="prettyprint lang-html" id="themer-pretty"></pre>').html(tlc)
  );
}
var primaryBackgroundTargets = [".ui-widget-header"];
 ...
 */
var themerSelectedTheme =
  $.cookie("themerSelectedTheme") != null ? $.cookie("themerSelectedTheme") : 0;
...
$.each(themerThemes, function (k, v) {
  themerCustomDefault[k] = { primaryColor: null };
});
var themerCustom =
  $.cookie("themerCustom") != null
    ? $.parseJSON($.cookie("themerCustom"))
    : themerCustomDefault;

if (themerThemes.length != themerCustom.length) {
  $.each(themerThemes, function (k, v) {
    if (typeof themerCustom[k] == "undefined") themerCustom[k] = v;
  });
  $.cookie("themerCustom", JSON.stringify(themerCustom));
}
  ...
  if ($("#themer").length) {
    ...
    $("#themer")
      .on("shown.bs.collapse", function () {
        $.cookie("themerOpened", 1);
        $(this).css("overflow", "visible");
      })
      .on("hidden.bs.collapse", function () {
        $.cookie("themerOpened", 0);
      })
    ...
    if (themerOpened == 1) $("#themer").collapse("show");
