        ...
        {foreach from=$addresses key='key' item='address_value'}
            {if $address_value['id_address'] == $selected_id_address}
                {if $settings['payment_address'][$p_address_key][$user_type]['display'] eq 1 || (isset($settings['payment_address'][$p_address_key]['conditional']) && $settings['payment_address'][$p_address_key]['conditional'] eq 1)}
                    {if $p_address_key eq 'dni' && $settings['payment_address'][$p_address_key][$user_type]['display'] == 0}
                        {$display_row = 'display:none;'}
                    {/if}
                    {* Start Changes to fix the issue of vat_number field not being shown when updating the payment address
                     * Replacing (if $p_address_key eq 'vat_number' && !$need_vat) condition with {if $p_address_key eq 'vat_number' && $settings['payment_address'][$p_address_key][$user_type]['display'] == 0}  
                     * NASep2023 vat_number_payment_update_form
                     * @date 26-09-2023
                            ...
                            {else}
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group ">
                                        <label class="control-label"for="payment_address[{$p_address_key}]">   
                                            {l s={$settings['payment_address'][$p_address_key]['title']} mod='supercheckout'}:
                                            {*
                                            Changes to add supercheckout-required in both cases required or not
                                            TGmar2023 Fix-validation
                                            @date 11-03-2023
                                            @author Tanisha Gupta
                                            *} 
                                            <span class="supercheckout-required" style="{if $p_address_key == 'vat_number'}
                                                {if $need_vat}display:inline;">*
                                                        {else}display:none;">
                                                        {/if}
                                                {else if $p_address_key == 'dni'}
                                                        {if $need_dni}display:inline">*
                                                        {else}display:none;">
                                                        {/if}
                                                {else}
                                                        display:
                                                        {if $settings['payment_address'][$p_address_key][$user_type]['require'] eq 1}
                                                                inline;">*
                                                        {else}
                                                                none;">
                                                        {/if}
