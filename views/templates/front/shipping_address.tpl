        ...
        <ul>
            <li>
                <div class="supercheckout-extra-wrap radio">
                    <input type="radio" name="shipping_address_value" value="0" id="shipping-address-existing"
                        checked="checked" />
                ...
                <div id="shipping-existing" class="styled-select">
                    <select name="shipping_address_id" class="form-control" style="">
                        {foreach from=$customer.addresses item='shipping_addr'}
                            <option value="{$shipping_addr['id']}" {if $shipping_addr['id'] == $id_address_delivery}
                                selected="selected" {/if}>{$shipping_addr['alias']}</option>
                        {/foreach}
                    </select>
                    <!-- Start: Code added by Anshul for adding update & remove address -->
                    <div id="kb_delivery_update_block" class="actionAddress">
                        <span id="delivery_address_edit" class="kb_edit_btn btn btn-warning"
                            onclick="updateAddressForm('delivery');"><i class="fa fa-edit"></i>
                            {l s='Update Address' mod='supercheckout'}</span>
                        <span id="delivery_address_delete" class="kb_delete_btn btn btn-danger"
                            onclick="deleteAddressForm('delivery');"><i class="fa fa-trash"></i>
                            {l s='Remove Address' mod='supercheckout'}</span>
                    </div>
                    <!-- End: Code added by Anshul for adding update & remove address -->
                    <div id="delivery_address_detail" class="supercheckout_address_detail card userAddressData">
                        <div class="card-body">
...
<div class="opc_shipping_address custom-panel">
    ...
    <div id="shipping-new"
        style="display: {if isset($guest_information)}block{else if $customer.addresses}none{else}block{/if};">
        {*            <div class="supercheckout-form billing-form" id="shipping_address_table" >*}
        ...
        {foreach from=$settings['shipping_address'] key='p_address_key' item='p_address_field'}
            ...
            {if $settings['shipping_address'][$p_address_key][$user_type]['display'] eq 1 || (isset($settings['shipping_address'][$p_address_key]['conditional']) && $settings['shipping_address'][$p_address_key]['conditional'] eq 1)}
                {if $p_address_key eq 'dni' && $settings['shipping_address'][$p_address_key][$user_type]['display'] == 0}
                ...
                {if $p_address_key eq 'id_state'}
                    <script>
                        var show_shipping_state = {$settings['shipping_address'][$p_address_key][$user_type]['display']};
                    </script>
                {/if}
                {if $p_address_key eq 'postcode'}
                    <script>
                        var show_shipping_postcode = {$settings['shipping_address'][$p_address_key][$user_type]['display']};
                    </script>
                ...
                {if $settings['shipping_address'][$p_address_key]['html_format'] == 1}
                    {if $p_address_key eq 'postcode'}
                        <div class="row sort_data" id="shipping_post_code"
                            data-percentage="{$settings['shipping_address'][$p_address_key]['sort_order']}" style="{$display_row}">
                        {else}
                            <div class="row sort_data" data-percentage="{$settings['shipping_address'][$p_address_key]['sort_order']}"
                                style="{$display_row}">
                            {/if}
                            <div class="col-sm-6">
                                <div class="form-group ">
                                    {* Start Changes for The font size of zipcode field text is smaller than the other field text. The font size was hardcoded set as 12px.
                                    * Replacing 12 by 14 to make the size same as other fields
                                    * NASep2023 zipcode_size_14
                                    * @date 26-09-2023
                                    * @modifier Nikhil Aggarwal *}
                                    <label class="control-label" for="shipping_address[{$p_address_key}]"
                                        {if $p_address_key eq "postcode"} style="font-size: 14px;"
                                        {* Changes end by Nikhil *}
                                        {/if}>{l s={$settings['shipping_address'][$p_address_key]['title']} mod='supercheckout'}:
                                        {if $p_address_key eq 'vat_number'}
                                            <span style="display:{if $need_vat}inline{else}none{/if};"
                                                class="supercheckout-required">*</span>
                                        {else if $p_address_key eq 'dni'}
                                            {* Start Changes to fix the issue of inline validation working on dni field even when its disabled
                                             * Showing the validation class only when the dni field is mandatory
                                             * @date 15-10-2024
                                             * @modifier Nikhil Aggarwal *}
                                            {* <span style="display:block" {if $need_dni}class="supercheckout-required"
                                                    {/if}>{if $need_dni}*{/if}</span> *}
                                            <span style="display:{if $need_dni}inline{else}none{/if};"
                                                class="supercheckout-required">*</span>
                                            {* Changes end by Nikhil *}
                                            {else}
                                               <span style="display:{if $settings['shipping_address'][$p_address_key]['guest']['require'] eq 1 
                                                || $settings['shipping_address'][$p_address_key]['logged']['require'] eq 1}inline{else}none{/if};"
                                                class="supercheckout-required">*</span>
                                            {/if}
                                        </label>
                                        {if $p_address_key eq 'id_country' || $p_address_key eq 'id_state'}
                                            ...
                                            <select name="shipping_address[{$p_address_key}]" class="supercheckout-large-field form-control"
                                                {if $p_address_key eq 'id_country'} onchange="restrictAutofillbyCountry(this)" {/if}>
                                                {if $p_address_key eq 'id_country'}
                                                    <option value="0">--</option>
                                                    {foreach from=$countries item='country'}
                                                        <option value="{$country['id_country']}" id='{$country['iso_code']}'
                                        ...
                                        {else if  $p_address_key eq 'other'}
                                            <input type="text" name="shipping_address[{$p_address_key}]" value=""
                                                class="supercheckout-large-field form-control">
                                        {else}

                                            {if $settings['google_auto_address']['enable'] eq 1}
                                                <input autocomplete="off" type="text"
                                                    {if $p_address_key eq 'address1'|| $p_address_key eq 'address2'}
                                                        placeholder="{l s='Enter a location' mod='supercheckout'}"
                                                    id='shipping_address_{$p_address_key|escape:'htmlall':'UTF-8'}' onFocus="geolocate()" {/if}
                                                    name="shipping_address[{$p_address_key|escape:'htmlall':'UTF-8'}]" value=""
                                                    class="form-control supercheckout-large-field {$google_region_type|escape:'htmlall':'UTF-8'} {if $p_address_key eq 'address1'|| $p_address_key eq 'address2'}autocomplete{/if}" />
                                            {else}
                                                <input autocomplete="off" type="text"
                                                    name="shipping_address[{$p_address_key|escape:'htmlall':'UTF-8'}]" value=""
                                                    class="supercheckout-large-field form-control" />
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group ">
                                        <label class="control-label"
                                            for="shipping_address[{$p_address_key}]">{l s={$settings['shipping_address'][$p_address_key]['title']}
                                            mod='supercheckout'}:
                                            {if $p_address_key eq 'vat_number'}
                                                <span style="display:{if $need_vat}inline{else}none{/if};"
                                                    class="supercheckout-required">*</span>
                                            {else if $p_address_key eq 'dni'}
                                                ...
                                                * @modifier Nikhil Aggarwal *}
                                                ...
                                                {else}
                                                    <span
                                                        style="display:{if $settings['shipping_address'][$p_address_key][$user_type]['require'] eq 1}inline{else}none{/if};"
                                                        class="supercheckout-required">*</span>
                                                {/if}
                                            </label>
                                            {if $p_address_key eq 'id_country' || $p_address_key eq 'id_state'}
                            ...
                            {else}
                                {if $p_address_key eq 'postcode'}
                                    <div class="sort_data" id="shipping_post_code"
                                        data-percentage="{$settings['shipping_address'][$p_address_key]['sort_order']}" style="{$display_row}">
                                    {else}
                                        <div class="sort_data" data-percentage="{$settings['shipping_address'][$p_address_key]['sort_order']}"
                                            style="{$display_row}">
                                        {/if}
                                        <div class="form-group ">
                                            <label class="control-label"
                                ...
                                <div class="supercheckout-blocks form-group">
                                    {if $field['type'] eq "textbox"}
                                        <label class="cursor_help"
                                            title="{$field['field_help_text']}">{$field['field_label']}{if $field['required'] eq "1"}<span
                                                style="display:inline;" class="supercheckout-required">*</span>{/if}</label>
                                        ...
                                        {foreach from=$field['options'] item=field_options}
                                            <div class="input-box input-field_{$field['id_velsof_supercheckout_custom_fields']}">
                                                <div class="checker checkbox" id="uniform-field_{$field['id_velsof_supercheckout_custom_fields']}">
                                                    <span class="checked">
                                                        <input {if $field_options['default_value'] eq $field_options['option_value']}checked{/if}
                                                            type="checkbox"
                                                            name="custom_fields[field_{$field['id_velsof_supercheckout_custom_fields']}][]"
                                                            value="{$field_options['option_value']}">
                                                        <label
                    ...
                    <ul>
                        <li>
                            <div class="input-box input-different-shipping"
                                {if !$settings['show_use_delivery_for_payment_add'][$user_type]} style="display:none;" {/if}>
                                {* Added a check of if allowed use_delivery_for_payment_add to uncheck the checked attribute of the checkbox
                                 * @author Prvind Panday
                                 * @date 08 March 2024
                                *}
                                <input type="checkbox" name="use_for_invoice" id="use_for_invoice"
                                    {if $is_shipping_address_same_as_billing && $settings['use_delivery_for_payment_add'][$user_type]}checked="checked" {/if}>
