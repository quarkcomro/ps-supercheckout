...
{block name='content'}
    ...
    {if !isset($empty)} 
        <script>
            /*
            Changes added to assign the stripe checkout url to the variable stripe_checkout_url
            @modifier Ashish Kumar
            @date 09-05-2024
            AKMay2024 stripe-checkout-compatible          
            */
            {if isset($stripe_checkout_url)}
                var stripe_checkout_url = "{$stripe_checkout_url}"; {*Feature:Spain DNI Check (Jan 2020)*}
            {/if}
            
            {if isset($email_require)}
                var email_require = "{$email_require|escape:'javascript':'UTF-8'}"; {*Feature:Spain DNI Check (Jan 2020)*}
            {/if}
            var ps_checkout_enable = {$ps_checkout_enable|escape:'javascript':'UTF-8'};
            var js_url = '{$store_url}';
            ...
            // changes done by kanishka to get value of google_auto_address_setting_enable
            var google_auto_address_setting_enable = "{$settings['google_auto_address']['google_auto_address_setting_enable']|escape:'htmlall':'UTF-8'}";
            // changes done by kanishka to get value of google_auto_address_setting_enable
                {/if}
            {/if}
                ...
                <fieldset class="group-select" id="supercheckout-fieldset">
                    {if !$logged} 
                        {if isset($mobileLoginActive) && ($mobileLoginActive eq 1)}
                            <div id="divkbmobilelogin" style="display:none" title="{l s='Login/Register With Mobile Number' mod='supercheckout'}" class="supercheckout-threecolumns divkbmobilelogin">
                                ...
                                {if isset($email_require) && $email_require == 1}
                                <div class="supercheckout-extra-wrap">
                    ...
                    <div class="supercheckout-threecolumns supercheckout-container supercheckout-skin-generic panel-body" id="supercheckout-columnleft">
                        <div class="row">
                            ...
                            {if isset($is_sc_mobile)}
                                {assign var='layout_name' value='1_column'}
                                {assign var='multiplier' value=1}
                            {/if}
                            {* end by dharmanshu for the exit popup module issue fix  7-11-2021 *}
                            ...
                            <div class="col-lg-4 col-md-4 col-sm-12 kb_mobile_sc_respo" {if $layout_name == '1_column'} style="width:100%;flex: 100%;max-width: 100%;" {else if $layout_name == '2_column' || $layout_name == '3_column'} style="width:{$settings['column_width'][$layout_name][1]*$multiplier}%;flex:{$settings['column_width'][$layout_name][1]*$multiplier}%;max-width:{$settings['column_width'][$layout_name][1]*$multiplier}%;" {/if}>
                                <div class="supercheckout-column-left columnleftsort checkout-section section-login custom-panel" id="columnleft-1" style="width:{100*$multiplier}%;flex:{100*$multiplier}%;max-width:{100*$multiplier}%;"> {*Need to change later*}
                                    ...
                                    <div class="supercheckout-blocks" data-column="{$settings['design']['shipping_address'][$layout_name]['column']|intval}" data-row="{$settings['design']['shipping_address'][$layout_name]['row']|intval}" data-column-inside="{$settings['design']['shipping_address'][$layout_name]['column-inside']|intval}">
                                        ...
                                        {*supercheckout enhacement start*}
                                        {if isset( $settings['enable_customer_profile'] ) && $settings['enable_customer_profile'] eq 1}
                                            {if isset( $settings['customer_profile_option'] ) && $settings['customer_profile_option'] eq 1}
                                               {if count($customer_profiles) > 0}
                                                   <div id="profile_customers_div" style="display: grid; padding: 15px 0px;">
                                                        <h2>{l s='Customer Profiles' mod='supercheckout'}</h2>
                                                        {foreach from=$customer_profiles item='profile'}
                                                            <div class="radio-inline" style='margin: 0px !important;'>
                                                <div class="radio">
                                                    <input type="radio" name="profile_customers" value="{$profile['id_profile']}" id="profile_customers_{$profile['id_profile']}"/>
                                                    <label for="profile_customers_{$profile['id_profile']}">{$profile['field_label']|escape:'htmlall':'UTF-8'}</label>
                                                </div>
                                            </div>
                                            ...
                                            {else}
                                                ...
                                                {if count($customer_profiles) > 0}
                                                    <div id="profile_customers_div" style="display: grid; padding: 15px 0px;"> 
                                                    <h2>{l s='Customer Profiles' mod='supercheckout'}</h2>
                                                    <select name="profile_customers" id="profile_customers">
{*                                                        <option value="">{l s='Select profile' mod='supercheckout'}</option>*} 
                                                        {foreach from=$customer_profiles item='profile'}
                                                            <option value="{$profile['id_profile']}">{$profile['field_label']}</option>
                                    ...
                                    <div style="{if $settings['payment_method']['enable'] eq 0}display:none;{/if}" class="supercheckout-blocks" data-column="{$settings['design']['payment_method'][$layout_name]['column']|intval}" data-row="{$settings['design']['payment_method'][$layout_name]['row']|intval}" data-column-inside="{$settings['design']['payment_method'][$layout_name]['column-inside']|intval}">
                                        <div class="opc_payment_method">
                                            ...
                                            <div id="payment_method_update_warning" class="supercheckout-checkout-content"></div>
                                                {* Code Modified by Shubham on 31-dec-2021 for making payment module compatible*}
                                            ...
                                            {else}  
                                                <div id="payment-method">
                                                    ...
                                                    {*
                                                     * Code Modified to show the payment methods on our page
                                    ...
                                    <div class="supercheckout-blocks confirmCheckoutBack" data-column="{$settings['design']['cart'][$layout_name]['column']|intval}" data-row="{$settings['design']['cart'][$layout_name]['row']|intval}" data-column-inside="{$settings['design']['cart'][$layout_name]['column-inside']|intval}" style="display:{if $settings['display_cart'] eq 1}block{else}none{/if};">
                                        ...
                                        {if isset($HOOK_SHOPPING_CART)}
                                                <div id="loyalty_text_holder">
                                                    {$HOOK_SHOPPING_CART nofilter}{*Variable contains html content, escape not required*}
                                                </div> 
                                        {/if}
                                        {*changes end*}    
                    ...
                    <input type="hidden" id="module_url" value="{$supercheckout_url|escape:'quotes'}" />    {*escape not required as contains html*}
                    <input type="hidden" id="addon_url" value="{$addon_url|escape:'quotes'}" />  {*escape not required as contains html*}
                    <input type="hidden" id="analytic_url" value="{$analytic_url|escape:'quotes'}" />    {*escape not required as contains html*}
                </fieldset>
            </div>
        </form>
        <div id="kb-tfauthorizedotnet-payment-form-div">
            <form id="tf-payment-form" action="" method="post">
                <input type="hidden" name="tf_information" value="" />
            </form>
        </div>
        {* changes by rishabh jain *}
        <!-- The Modal -->
        ...
        <script>
            var kb_customer_profiles_data = '{json_encode($customer_profiles)|addslashes nofilter}{*escape not required as contains html*}';
            var kb_customer_profiles = JSON.parse(kb_customer_profiles_data);
            var kb_profiles_mapping_data = '{json_encode($profile_mapping_data)|addslashes nofilter}{*escape not required as contains html*}';
            var kb_profiles_mapping = JSON.parse(kb_profiles_mapping_data);
                                ...
                                autocompleteLsr[name] = google.maps.event.addListener(autocomplete[name], 'place_changed', function () {
                                    ...
                                    for (var i = 0; i < place.address_components.length; i++) {
                                        ...
                                        if (typeof field[addressType] !== 'undefined') {
                                            var val = place.address_components[i][field[addressType]];
                                            // changes done by kanishka to combine address into Address Line 1 and Address Line 2
                                            /**
                                             * Start Changes to fix the issue of address being broken even if Enable to Divide the Google Auto Address setting is disabled.
                                             * Issue is occurring because we are checking Number(google_auto_address_setting_enable)
                                             * Above condition gives true for the 0 also. So, replacing it with google_auto_address_setting_enable == '0'
                                             * NASep2023 google_auto_address_setting_enable
                                             * @date 26-09-2023
                                             * @modifier Nikhil Aggarwal
                                            */
                                            if (google_auto_address_setting_enable == '0') {
                                                // Changes end by Nikhil
                                                if(addressType=='route') {
                                                    var address_type_new = 'street_number';                                
                                                    var shipping_address_new_value = $('#'+wraps.form+' .'+address_type_new).val();
                                                    var shipping_address_combine = val.concat(', '+shipping_address_new_value);
                                                    $('#'+wraps.form+' .'+address_type_new).val(shipping_address_combine);
                                                 } else {
