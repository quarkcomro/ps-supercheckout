{if isset($supercheckout_gdpr_setting) && !empty($supercheckout_gdpr_setting)}
    <div id="supercheckout-policy">
        {foreach from=$supercheckout_gdpr_setting item='gdpr_setting'}
            <div class="checkbox">
                    {*  Added a custom data attribute to check if the policy is marked as requried or not.
                    * @modifier Manish
                    * @date 28-04-2026
                    * MPAPR2026 
                    *}
               
                     <input id="kb_super_policy_{$gdpr_setting['policy_id']|intval}" {if $gdpr_setting['is_manadatory']} data-required="1" {else} data-required="0" {/if} class="" type="checkbox" name="kb_super_policy[{$gdpr_setting['policy_id']|intval}]" value="1"/>
                     <label>
                    {* Start - Code Modified by Priyanshu on 14-June-2019  to fix the issue of Privacy Policy URL field *}
                    {if $gdpr_setting['url'] neq '' && $gdpr_setting['url'] neq '0'} {*by dharmanshu for 1-11-2021 for the privacy policy URL issue*}
